...
double DPEngine::calculate_dark_cost(const Venue* venue, int64_t quantity, const std::vector<double>& lit_dp_table) const {
    const VenueConfig& venue_config = venue->get_config();
...
SplitResult DPEngine::compute_optimal_split(int64_t total_size, Side side, int64_t worst_price, const std::vector<Venue*>& venues) {
    int64_t num_lots = total_size / config.lot_size;

    std::vector<std::pair<Venue*, int>> lit_venues;
    std::vector<std::pair<Venue*, int>> dark_venues;

    for (int i = 0; i < venues.size(); ++i) {
        if (venues[i]->get_type() == LIT) {lit_venues.push_back({venues[i], i});} else {dark_venues.push_back({venues[i], i});}
    }
    ...
    for (int k = 1; k <= lit_venues.size(); ++k) {
        Venue* venue = lit_venues[k-1].first;
    ...
    for (int k = lit_venues.size() + 1; k <= venues.size(); ++k) {
        Venue* venue = dark_venues[k - lit_venues.size() - 1].first;
